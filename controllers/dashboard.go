...
func (d Dashboard) DeleteUser(c echo.Context) error {
	if isAdmin := cookies.GetApp(c).IsAdmin; !isAdmin {
	...
	if flashErr := cookies.AddFlash(c, cookies.FlashSuccess, "User deleted successfully"); flashErr != nil {
		return flashErr
	}
...
func (d Dashboard) MakeUserAdmin(c echo.Context) error {
	...
	if err != nil {
		if flashErr := cookies.AddFlash(c, cookies.FlashError, fmt.Sprintf("Failed to make user admin: %v", err)); flashErr != nil {
			return flashErr
		}
	}

	if flashErr := cookies.AddFlash(c, cookies.FlashSuccess, "Admin status updated successfully"); flashErr != nil {
		return flashErr
	}
